...
function Header({
  ...
  // keep dropdown positioned via viewport coords to avoid clipping by parent containers
  useEffect(() => {
    if (menuOpen && buttonRef.current) {
      const rect = buttonRef.current.getBoundingClientRect();
      const menuWidth = 240; // w-60 => 15rem
      setMenuPos({
  ...
  return (
    <header className="sticky top-0 z-30" style={{ backgroundColor: 'var(--bg-page)', color: 'var(--text-primary)', borderBottom: `1px solid var(--border-subtle)`  }}>
      <div className="">
        <div className="flex items-center justify-between h-16" >
          ...
          <div className="relative flex items-center" ref={menuRef} >
            ...
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-full text-white text-sm font-bold" style={{ backgroundColor: 'var(--blue-primary)', cursor: 'pointer' }}>
                {(displayName || 'U')[0]?.toUpperCase()}
              </span>
              <span className="text-sm font-semibold hidden sm:inline" style={{ color: 'var(--text-primary)', cursor: 'pointer' }}>
                {displayName}
              </span>
              <svg
